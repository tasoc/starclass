...
def validate(labels_test, features_test):
	labels_test_val = []
...
if __name__ == '__main__':
	...
	parser.add_argument('-cv', '--KFoldCV', help='Enable KFold cross-validation with 5 folds (only relevant if --train activated)', type=bool, default=False)
	...
	if args.train:
		...
		if args.train == 'tdasim':
			tset = training_sets.tda_simulations(datalevel=args.datalevel, tf=args.testfraction, KFoldCV=args.KFoldCV, classifier=current_classifier)
		elif args.train == 'keplerq9':
			tset = training_sets.keplerq9(datalevel=args.datalevel, tf=args.testfraction, KFoldCV=args.KFoldCV, classifier=current_classifier)
		elif args.train == 'keplerq9-linfit':
			tset = training_sets.keplerq9linfit(datalevel=args.datalevel, tf=args.testfraction, KFoldCV=args.KFoldCV, classifier=current_classifier)

		# Name output classifier file
		clfile = current_classifier + '_' + str(np.round(args.testfraction,decimals=2))


		# This is done to enable the test set of each validation fold to be written
		# To database for meta classifier to use
		...
		with classifier(level=args.level, tset=args.train, features_cache=tset.features_cache, clfile=clfile, cv=args.KFoldCV) as stcl:
			logger.debug("Starting training...")

			if args.KFoldCV == True:

				for idx, (train_idx, test_idx) in enumerate(tset.skf):

					tset.cvtrain_idx = tset.train_idx[train_idx]
					tset.cvtest_idx = tset.train_idx[test_idx]
					...
					labels_test, y_pred = validate(labels_test, features_test)

					y_pred = np.array(y_pred)
					# Compare to known labels - ADD SAVING PROCESS, JUST PRINTS TO LOG
					acc = accuracy_score(labels_test, y_pred)
					logger.info('Fold {}, Accuracy: {}'.format(idx, acc))

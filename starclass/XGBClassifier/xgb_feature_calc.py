...
def feature_extract_single(features,linflatten = False):
    """
    ...
    freq = features['freq1']
    folded_lc = phase_fold_lc(lc, freq)
    ...
    amp_ratios = freq_ampratios(obj,usedfreqs)
    phasediffs = freq_phasediffs(obj,usedfreqs)

    features_dict['PeriodLS'] = periods[0]
    features_dict['Freq_amp_0'] = obj['amp'+str(usedfreqs[0]+1)]
    for f in np.arange(2):
        features_dict['Freq_ampratio_'+str(f+1)] = amp_ratios[f]
        features_dict['Freq_relphase_'+str(f+1)] = phasediffs[f]

...
class CorrectorQualityFlags(QualityFlagsBase):
	...
	"""
	FlaggedBadData = 1 #: Bad data based on pixel flags
	ManualExclude = 2 #: Manual exclude
	SigmaClip = 4 #: Point removed due to sigma clipping
	JumpAdditiveConstant = 8 #: Jump corrected using additive constant
	JumpAdditiveLinear = 16 #: Jump corrected using additive linear trend
	JumpMultiplicativeConstant = 32 #: Jumb corrected using multiplicative constant
	JumpMultiplicativeLinear = 64 #: Jump corrected using multiplicative linear trend
	Interpolated = 128 #: Point is interpolated
	Rejected = 256 #: Point has been rejected in processing

	# Default bitmask
	DEFAULT_BITMASK = (FlaggedBadData | ManualExclude | Rejected)
